...
router.post('/signup', async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Cannot proceed your registration data, please try again.', error: err });
  }
});
...
router.post('/signin', async (req, res, next) => {
  ...
  try {
    ...
    user.comparePassword(password, (err, isMatch) => {
      if (!isMatch) res.status(401).json({ success: false, message: 'Password did not match.' });
