...
mockmongo, mongoose, jwt, opts, supertest, server,
...
describe('# Testing Book Routes', () => {
  ...
  before(done => {
    mockmongo
      ...
      .then(() => {
        ...
        // const category = new Category({ categoryName: 'Politic' });
        // category.save();

        // == get author id to save temporary book ==
        Author.findOne({}).exec((err, author) => {
          if (err) done(err);
    ...
    it('returns a unknown server error when trying to find book with id', done => {
      ...
      supertest(server)
        ...
        .end((err, res) => {
          expect(res.body)
            .to.be.an('object')
            .that.has.all.keys('success', 'message', 'error');
          expect(res.body)
            .to.have.property('success')
            .equal(false);
          expect(res.body)
  ...
  // TODO: change the following testing according to the route file
  ...
  describe('GET /api/v1/books/synopsis/{bookid}', () => {
    it('returns a book synopsis {no apiKey required}', done => {
      // == create a fake book ==
      const book = new Book({
        title: 'Book V',
        category: categoryid,
    ...
    it('returns a message unknown server error when trying to save new book', done => {
      ...
      supertest(server)
        ...
        .end((err, res) => {
          ...
          expect(res.body.results)
            .to.be.an('object')
            .that.has.all.keys('errors', '_message', 'message', 'name');
          expect(res.body.results)
      ...
      Book.findOne({}).exec((err, book) => {
        supertest(server)
          ...
          .end((error, res) => {
            ...
            expect(res.body.results)
              .to.be.an('object')
              .that.has.all.keys('_id', 'title', 'category', 'pages', 'author', 'published', 'synopsis');
            expect(res.body.results)
              .to.have.property('_id')
              .to.be.a('string')
              .to.have.lengthOf(24);
      ...
      supertest(server)
        ...
        .end((err, res) => {
          ...
          expect(res.body.error)
            .to.be.an('object')
            .that.has.all.keys('message', 'name', 'stringValue', 'kind', 'value', 'path');
          expect(res.body.error)
            .to.have.property('message')
            .to.be.a('string');
          expect(res.body.error)
    ...
    it('returns a message failed to delete a book', done => {
      ...
      supertest(server)
        ...
        .end((err, res) => {
          expect(res.body)
            .to.be.an('object')
            .that.has.all.keys('success', 'message');
          expect(res.body)
            .to.have.property('success')
            .equal(false);
          expect(res.body)
